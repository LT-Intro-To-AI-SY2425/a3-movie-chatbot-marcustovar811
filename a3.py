...
def title_by_director(matches: List[str]) -> List[str]:
    ...
    """
    directorName = matches[0]
    result = []
    for movie in movie_db:
        if(get_director(movie) == directorName):
            result.append(get_title(movie))
    return result
...
def actors_by_title(matches: List[str]) -> List[str]:
    ...
    """
    movieTitle = matches[0]
    result = []
    for movie in movie_db:
        if(get_title(movie) == movieTitle):
...
def title_by_actor(matches: List[str]) -> List[str]:
    ...
    result = []
    for movie in movie_db:
        if(get_actors(movie).count(actorName) != 0):
            result.append(get_title(movie))
    return result
...
def search_pa_list(src: List[str]) -> List[str]:
    ...
    for pattern, action in pa_list:
        matched_words = match(pattern, src)
        if matched_words is not None:
            result = action(matched_words)
